...
class ProviderManager:
    ...
    def _is_provider_available(cls, provider_name: str) -> bool:
        ...
        # For code companions, check if their CLI tools are available
        if provider_name == "copilot":
            return cls._check_cli_available(["gh", "copilot", "--version"])
        elif provider_name == "cursor":
            return cls._check_cli_available(["cursor", "--version"])
        elif provider_name == "codeium":
            return cls._check_cli_available(["codeium", "--version"])
        elif provider_name == "tabnine":
            return cls._check_cli_available(["tabnine", "--version"])
        
        # For API providers, assume available if registered
        # (actual API key validation happens at runtime)
        return True
