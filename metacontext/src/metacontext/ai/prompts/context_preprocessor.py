...
DOMAIN_KEYWORDS = {
    "machine_learning": [
        "model", "training", "dataset", "features", "algorithm", "neural", "deep learning",
        "classification", "regression", "clustering", "validation", "accuracy", "loss",
        "hyperparameter", "epoch", "batch", "gradient", "optimizer", "tensorflow", "pytorch",
    ],
    "data_science": [
        "analysis", "visualization", "pandas", "numpy", "statistics", "correlation",
        "distribution", "outlier", "preprocessing", "cleaning", "eda", "exploration",
    ],
    "geospatial": [
        "gis", "spatial", "coordinate", "projection", "raster", "vector", "geometry",
        "map", "location", "latitude", "longitude", "crs", "geojson", "shapefile",
    ],
    "web_development": [
        "api", "endpoint", "route", "middleware", "database", "authentication", "session",
        "request", "response", "http", "rest", "graphql", "frontend", "backend",
    ],
    "configuration": [
        "config", "settings", "environment", "variable", "parameter", "option",
        "default", "override", "production", "development", "staging",
    ],
}
...
def filter_content_by_keywords(content: str, keywords: list[str], min_relevance: float = 0.1) -> str:
    ...
    for chunk in chunks:
        ...
        if relevance >= min_relevance or len(chunk) < 100:  # Always include short chunks
            relevant_chunks.append(chunk)
    
    return '\n\n'.join(relevant_chunks)


def extract_file_type_specific_content(content: str, file_path: str, max_length: int) -> str:
    ...
    if 'readme' in file_name:
        readme_keywords = ['description', 'usage', 'installation', 'getting started', 'overview', 'purpose']
        filtered_content = filter_content_by_keywords(content, readme_keywords, min_relevance=0.05)
    ...
    # Configuration files - extract business logic indicators
        ...
        return filter_content_by_keywords(content, config_keywords, min_relevance=0.1)[:max_length]
    
    # Code files - extract signatures and docstrings
    elif file_ext in {'.py', '.js', '.ts', '.java', '.cpp', '.c', '.h', '.rs', '.go'}:
        return extract_code_signatures(content, max_length)
    
    # Documentation files
    elif file_ext in {'.md', '.rst', '.txt'}:
        doc_keywords = ['overview', 'architecture', 'design', 'implementation', 'usage', 'api']
        filtered_content = filter_content_by_keywords(content, doc_keywords, min_relevance=0.05)
        return extract_important_sections(filtered_content, max_length)
...
def extract_important_sections(content: str, max_length: int = MAX_FILE_CONTENT_LENGTH) -> str:
    ...
    for line in lines:
        ...
        is_structural = (
            line.strip().startswith('#') or  # Markdown headers
            line.strip().startswith('def ') or  # Python functions
            line.strip().startswith('class ') or  # Python classes
            line.strip().startswith('function ') or  # JavaScript functions
            '=' in line and len(line.strip()) < 100  # Configuration entries
        )
...
def extract_code_signatures(content: str, max_length: int = MAX_FILE_CONTENT_LENGTH) -> str:
...
def preprocess_file_contents(
    ...
    for file_path, content in prioritized_files.items():
        ...
        processed_content = extract_file_type_specific_content(content, file_path, per_file_limit)
