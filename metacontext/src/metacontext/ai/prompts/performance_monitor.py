...
class PromptMetrics:
    ...
    def limit_compliance_ratio(self) -> float:
        ...
        if not self.character_limit:
            return 1.0
        return min(1.0, self.character_limit / self.output_char_count) if self.output_char_count > 0 else 1.0
...
class PerformanceMonitor:
    ...
    def _load_metrics(self) -> None:
        ...
        if self.metrics_file and self.metrics_file.exists():
            try:
                ...
                logger.info(f"Loaded {len(self.metrics)} metrics from {self.metrics_file}")
    ...
    def _save_metrics(self) -> None:
        ...
        if self.metrics_file:
            try:
                ...
                logger.debug(f"Saved {len(self.metrics)} metrics to {self.metrics_file}")
            except Exception as e:
    ...
    def get_template_statistics(self, template_name: str) -> dict[str, Any]:
        ...
        return {
            ...
            "efficiency_ratio": sum(m.efficiency_ratio for m in template_metrics) / len(template_metrics),
        }
    ...
    def get_overall_statistics(self) -> dict[str, Any]:
        ...
        template_names = list(set(m.template_name for m in self.metrics))
        ...
        overall_compliance = sum(m.within_limit for m in self.metrics) / len(self.metrics)
...
def create_performance_report(monitor: PerformanceMonitor) -> str:
    ...
    if template_names:
        ...
        for template_name in template_names:
            ...
            report.append(f"  Avg Processing Time: {stats['avg_processing_time_ms']:.1f}ms")
